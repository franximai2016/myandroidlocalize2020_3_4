...
public class CheckLicense {
    ...
    private static final String PRODUCT_CODE = "PANDROIDLOCALIZ";
    ...
    public static void requestLicense(String message) {
        ApplicationManager.getApplication().invokeLater(() ->
                showRegisterDialog(PRODUCT_CODE, message), ModalityState.NON_MODAL);
    }

    private static void showRegisterDialog(String productCode, String message) {
        ActionManager am = ActionManager.getInstance();
        AnAction action = am.getAction("RegisterPlugins");
        if (action == null) action = am.getAction("Register");

        if (action != null) {
            DataContext dc = asDataContext(productCode, message);
            ActionUtil.invokeAction(action, dc, "", null, null);
        }
    }

    @NotNull
    private static DataContext asDataContext(String code, String message) {
        return dataId -> {
            if ("register.product-descriptor.code".equals(dataId)) return code;
            if ("register.message".equals(dataId)) return message;
            return null;
        };
    }
    ...
    private static boolean isKeyValid(String key) {
        try {
            ...
            byte[] licBytes = Base64.getMimeDecoder().decode(parts[1]);
            byte[] sigBytes = Base64.getMimeDecoder().decode(parts[2]);
            byte[] certBytes = Base64.getMimeDecoder().decode(parts[3]);

            Signature sig = Signature.getInstance("SHA1withRSA");
            sig.initVerify(createCertificate(certBytes, Collections.emptyList(), false));

            sig.update(licBytes);
            if (!sig.verify(sigBytes)) return false;

            String licData = new String(licBytes, StandardCharsets.UTF_8);
            return licData.contains("\"licenseId\":\"" + licenseId + "\"");
    ...
    private static boolean isLicenseServerStampValid(String stamp) {
        try {
            ...
            byte[] certBytes = base64.decode(parts[5]);

            List<byte[]> intermediates = new ArrayList<>();
            for (int i = 6; i < parts.length; i++) {
                intermediates.add(base64.decode(parts[i]));
            }

            Signature sig = Signature.getInstance(signatureType);
            sig.initVerify(createCertificate(certBytes, intermediates, true));
            ...
            return expectedMachineId.equals(machineId) && diff < TIMESTAMP_VALIDITY_PERIOD_MS;
    ...
    private static X509Certificate createCertificate(
            byte[] certBytes, Collection<byte[]> intermediates, boolean checkDate
    ) throws Exception {
        ...
        all.add(cert);
        for (byte[] bytes : intermediates) {
            all.add(factory.generateCertificate(new ByteArrayInputStream(bytes)));
        }

        Set<TrustAnchor> trustAnchors = new HashSet<>();
        for (String rc : ROOT_CERTIFICATES) {
            X509Certificate root = (X509Certificate)
                    factory.generateCertificate(new ByteArrayInputStream(rc.getBytes(StandardCharsets.UTF_8)));
        ...
        CertPath path = CertPathBuilder.getInstance("PKIX").build(params).getCertPath();
